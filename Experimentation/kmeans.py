...
def k_means(X, n_clusters, init, n_init, algorithm, max_iter=100):
    # Nombre de clusters = valeur maximale du résultat dans le fichier .txt (np.max(X["Résultat"]))
    start_time=time.time()
    kmeans = KMeans(n_clusters=n_clusters, init=init, n_init=n_init, max_iter=max_iter, algorithm=algorithm).fit(X.drop(columns=["Résultat"]))
    end_time=time.time()
    ...
    print("Le coefficient de silhouette est ", silhouette_score(X.drop(columns=["Résultat"]), labels = labels, metric='euclidean'))
    print("L'indice de rand ajusté est ", adjusted_rand_score(X["Résultat"], labels))
    print("L'indice de Davies-Bouldin est", davies_bouldin_score(X.drop(columns=["Résultat"]), labels))

    return kmeans.inertia_, labels, end_time-start_time

# k_means(jain, 2, 'random', 1, 'lloyd')
# k_means(jain, 2, 'random', 20, 'lloyd')
# k_means(jain, 2, 'k-means++', 1, 'lloyd')
# k_means(jain, 2, 'k-means++', 20, 'lloyd')
# k_means(jain, 2, 'random', 1, 'elkan')
# k_means(jain, 2, 'random', 20, 'elkan')
# k_means(jain, 2, 'k-means++', 1, 'elkan')
# k_means(jain, 2, 'k-means++', 20, 'elkan')
# k_means(aggregation, 7, 'random', 1, 'lloyd')
# k_means(aggregation, 7, 'random', 20, 'lloyd')
# k_means(aggregation, 7, 'k-means++', 1, 'lloyd')
# k_means(aggregation, 7, 'k-means++', 20, 'lloyd')
# k_means(aggregation, 7, 'random', 1, 'elkan')
# k_means(aggregation, 7, 'random', 20, 'elkan')
# k_means(aggregation, 7, 'k-means++', 1, 'elkan')
# k_means(aggregation, 7, 'k-means++', 20, 'elkan')
# k_means(pathbased, 3, 'random', 1, 'lloyd')
# k_means(pathbased, 3, 'random', 20, 'lloyd')
# k_means(pathbased, 3, 'k-means++', 1, 'lloyd')
# k_means(pathbased, 3, 'k-means++', 20, 'lloyd')
# k_means(pathbased, 3, 'random', 1, 'elkan')
# k_means(pathbased, 3, 'random', 20, 'elkan')
# k_means(pathbased, 3, 'k-means++', 1, 'elkan')
# k_means(pathbased, 3, 'k-means++', 20, 'elkan')
